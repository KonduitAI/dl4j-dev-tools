...
class TestTensorflowIR {
    ...
    fun runTfImportProcess() {
        ...
        val mappingProcesses = importProcess.createMappingProcesses(graph = tfGraph).map {
                process -> TensorflowImportContext(process = process,mappingContext = importProcess.createMappingContext(graph = tfGraph,node = tensorflowNode))
        }
    ...
    fun testTensorflowConv2d() {
        ...
        /**
         * TODO: resolve wrong data types being added by comparing
         * initFromTensorflow in old method to new methods
         * Methods discovered wrong so far:
         * Conv2D
         * MaxPool
         *
         * Both add attributes to float for some reason when all should be integers.
         * It's likely a rule sitting in the mapping process.
         *
         */
        println(importedGraph)
